...
func Increment(port Port, octets int, packetID, octetID fwdpb.CounterId) {
	...
	port.Increment(octetID, uint32(octets)+4)
}
...
func Output(port Port, packet fwdpacket.Packet, dir fwdpb.PortAction, _ *fwdcontext.Context) (err error) {
	...
	if mac[0] == 0xFF && mac[1] == 0xFF && mac[2] == 0xFF && // Broadcast address is when all bits are set to 1.
	...
	} else if mac[0]%2 == 0 { // Unicast address is when is least significant bit of the 1st octet is 0.
